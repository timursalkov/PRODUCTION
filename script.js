...
function toastMessage(text){toast.textContent=text;toast.classList.add('show');clearTimeout(window.toastTimer);window.toastTimer=setTimeout(()=>toast.classList.remove('show'),3000);}
...
function showGame(){
  stopRolling();clearHash();hero.hidden=true;final.hidden=true;game.hidden=false;top();
}
function showHero(){
  stopRolling();clearHash();game.hidden=true;final.hidden=true;hero.hidden=false;top();
}
function resetGame(){
...
function rollDice(){
  if(rolling||!rollBtn||rollBtn.disabled)return;
  rolling=true;
...
acceptBtn.addEventListener('click',event=>{event.preventDefault();showGame();});
laterBtn.addEventListener('click',()=>{laterBtn.textContent='ЛАДНО, Я ПОДОЖДУ ♡';toastMessage('Хорошо. Но кубики уже приготовлены 🎲');});
...
timeButtons.forEach(btn=>btn.addEventListener('click',async()=>{
  ...
  game.hidden=true;final.hidden=false;clearHash();top();
...
copyBtn.addEventListener('click',async()=>{
  ...
  if(ok){copyBtn.textContent='✓ СКОПИРОВАНО';toastMessage('Текст скопирован в буфер обмена 💌');}
  else toastMessage('Не удалось скопировать. Попробуй ещё раз.');
});
telegramBtn.addEventListener('click',()=>{if(selectedTime)telegramBtn.href=telegramUrl(makeMessage(selectedTime));});
backBtn.addEventListener('click',showHero);
restartBtn.addEventListener('click',()=>{resetGame();top();});
